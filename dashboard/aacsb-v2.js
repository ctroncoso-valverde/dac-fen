...
/* Overrides: setAAView, buildAA, buildAATrack, upAASt, addAANote, renderAANotes */
/* Removes: buildAABaseline, renderAACommits, buildAAExec */
(function(){
var aaGroupBy='std';
...
var h='<div style="font-size:16px;font-weight:600;margin-bottom:12px">Métricas clave</div>';
h+='<div style="border:1px solid var(--bd);border-radius:10px;overflow:hidden;background:var(--bg3);margin-bottom:20px"><table style="width:100%;border-collapse:collapse;font-size:14px">';
h+='<tr style="background:var(--bg2)"><th style="text-align:left;padding:10px 14px;font-size:13px;font-weight:600;color:var(--tx2)">Métrica</th><th style="text-align:right;padding:10px 10px;font-size:13px;color:var(--tx2)">Base iSER</th><th style="text-align:center;padding:10px 4px;color:var(--tx3)"></th><th style="text-align:right;padding:10px 10px;font-size:13px;color:var(--tx2)">A la fecha</th><th style="text-align:right;padding:10px 10px;font-size:13px;color:var(--tx2)">Delta</th><th style="text-align:right;padding:10px 14px;font-size:13px;color:var(--tx2)">Meta</th></tr>';
...
var dCol=delta>0?'var(--donet)':delta<0?'var(--nonet)':'var(--tx3)';
var dTxt=delta>0?'\u25b2 '+delta+'pp':delta<0?'\u25bc '+Math.abs(delta)+'pp':'\u2014 0pp';
var curCol=cur>=tgt?'var(--donet)':'var(--tx)';
h+='<tr><td style="padding:10px 14px;border-top:1px solid var(--bg2);font-weight:500">'+AA_METRIC_NAMES[k]+'</td>';
h+='<td style="text-align:right;padding:10px;border-top:1px solid var(--bg2);color:var(--tx2)">'+base+'%</td>';
h+='<td style="text-align:center;padding:10px 4px;border-top:1px solid var(--bg2);color:var(--tx3)">\u2192</td>';
h+='<td style="text-align:right;padding:10px;border-top:1px solid var(--bg2);font-weight:700;color:'+curCol+'">'+cur+'%</td>';
h+='<td style="text-align:right;padding:10px;border-top:1px solid var(--bg2);font-size:12px;color:'+dCol+'">'+dTxt+'</td>';
h+='<td style="text-align:right;padding:10px 14px;border-top:1px solid var(--bg2);color:var(--tx3);font-size:13px">\u2265'+tgt+'%</td></tr>';
});
h+='</table><div style="padding:10px 14px;background:var(--bg2);font-size:12px;color:var(--tx2);line-height:1.6"><b>AoL postgrado ('+AA_BASELINE.aolPost+'%):</b> '+AA_AOL_POST_NOTE+'<br>SA, Qualified, Participating y P con CI se actualizan desde el snapshot Faculty data. AoL se actualiza manualmente.</div></div>';

// Criticality cards
h+='<div style="font-size:16px;font-weight:600;margin-bottom:12px">Compromisos por criticidad</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px">';
...
h+='<div style="background:var(--bg3);border:1px solid var(--bd);border-radius:10px;padding:16px"><div style="font-size:14px;font-weight:600;color:'+CRIT_TX[c]+';margin-bottom:12px">'+CRIT_LABELS[c]+'</div>';
h+='<div style="display:flex;justify-content:space-between;align-items:baseline"><div><div style="font-size:12px;color:var(--tx3)">Base</div><div style="font-size:24px;font-weight:700">'+total+'</div></div><div style="font-size:18px;color:var(--tx3)">\u2192</div><div style="text-align:right"><div style="font-size:12px;color:var(--tx3)">Pendientes</div><div style="font-size:24px;font-weight:700;color:'+CRIT_TX[c]+'">'+pend+'</div></div></div>';
h+='<div style="margin-top:10px;height:6px;background:'+CRIT_BG[c]+';border-radius:3px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:var(--donet);border-radius:3px"></div></div>';
h+='<div style="font-size:12px;color:var(--tx3);margin-top:4px">'+done+' de '+total+' completados</div></div>';
});
h+='</div>';

// Rings by standard
h+='<div style="font-size:16px;font-weight:600;margin-bottom:12px">Avance por est\u00e1ndar</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px">';
for(var s=0;s<9;s++){
...
window.upAASt=function(id,v){
if(!aaState.commits[id])aaState.commits[id]={status:v,notes:[]};
else aaState.commits[id].status=v;
saveAAState();buildAATrack2();buildAAResumen();
};

window.addAANote=function(id){
var inp=document.getElementById('aan_'+id);var t=inp.value.trim();
if(!t)return;
if(!aaState.commits[id])aaState.commits[id]={status:getAASt(id),notes:[]};
if(!aaState.commits[id].notes)aaState.commits[id].notes=[];
aaState.commits[id].notes.push({date:new Date().toLocaleDateString('es-CL'),text:t});
saveAAState();inp.value='';
document.getElementById('aanl_'+id).innerHTML=renderAANotes(id);
};

window.renderAANotes=function(id){
var n=aaState.commits[id]?aaState.commits[id].notes:[];
if(!n||!n.length)return'<div style="color:var(--tx3);font-style:italic;padding:3px 0;font-size:12px">Sin notas</div>';
return n.slice().reverse().map(function(x){return'<div class="aa-note-entry" style="font-size:12px"><b>'+x.date+'</b> \u2014 '+x.text+'</div>'}).join('');
};
